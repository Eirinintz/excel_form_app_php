...
class LibraryController extends Controller
{
    ...
    public function people(Request $request)
    {
        ...
        if ($search !== '') {
            ...
            } elseif (in_array($searchCategory, ['hmeromhnia_eis','titlos','syggrafeas','ekdoths','etosEkdoshs','ISBN'], true)) {
...
public function resolveDuplicates()
{
    ...
    if (empty($duplicates) && empty($potential)) {
        return redirect()
            ->route('upload')
            ->with('status', 'No duplicates or potential insertions to resolve.');
    }
   ...
   public function replaceSelected(Request $request)
{
    ...
    $selectedDup = array_map('intval', $request->input('duplicate_ids',[]));
    $selectedIns = array_map('intval', $request->input('insertion_ids',[]));
   ...
   // only DB duplicates
    if (($dup['type'] ?? null) !== 'database') {
        continue; // ⛔ skip file duplicates
    }

    $ari8mos = (int) ($dup['ari8mos']?? 0);

    
    if (!$ari8mos || !in_array($ari8mos, $selectedDup, true)) {
        continue;
    }
    ...
    private function personRules(): array
    {
        return [
            ...
            'syggrafeas' => ['nullable', 'string'],
            'koha' => ['nullable', 'string'],
            'titlos' => ['nullable', 'string'],
